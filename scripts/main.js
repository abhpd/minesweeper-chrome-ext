...
async function gameWonAndCookie() {
  ...
  cookie.addEventListener("click", () => {
    ...
    const random_cookie_quote =
      cookie_quote_array[Math.floor(Math.random() * cookie_quote_array.length)];
...
document
  ...
  .addEventListener("click", (event) => {
    ...
    chrome.storage.local.get("mineCount", (result) => {
      document.getElementById("mines-count-input").value = result.mineCount
        ? result.mineCount
        : default_mine_count;
    });
    ...
    chrome.storage.local.get("mineCount", (result) => {
      if (result.mineCount) {
        document.getElementById("auto-miner-input").setAttribute("checked", "");
      }
    });
...
document
  ...
  .addEventListener("click", async () => {
    ...
    const newAutoMiner = document.getElementById("auto-miner-input").checked;
  ...
  document.querySelectorAll(".curtain-cell").forEach((element) => {
    element.addEventListener("click", async (event) => {
      ...
      chrome.storage.local.get("mine_grid", (result) => {
        if (result.mine_grid.find((ele) => ele[0] === row && ele[1] === col)) {
          gameEndAndReveal();
        }
      });
      ...
      chrome.storage.local.get("clicked_grid", (result) => {
        if (
          !result.clicked_grid.find((ele) => ele[0] === row && ele[1] === col)
        ) {
          chrome.storage.local.get("flag_grid", (flag_result) => {
            if (
              ...
              chrome.storage.local.set(
                ...
                () => {
                  var curtain_cells =
                    document.querySelectorAll(".curtain-cell");
                  ...
                  chrome.storage.local.get("score", (result_score) => {
                    const win_check_clicked = result_score.score;
                    chrome.storage.local.get(
                      "mineCount",
                      (result_mineCount) => {
                        const win_check_mine_count = result_mineCount.mineCount;
                        ...
                        if (
                          Math.floor(win_check_clicked) +
                            Math.floor(win_check_mine_count) ===
                          130
                        ) {
                          console.log(
                            `WIIIIIINNNNNN :${
                              win_check_clicked + win_check_mine_count
                            }`
                          );
                          chrome.storage.local.set({ gameRunning: -1 }, () => {
                            gameWonAndCookie();
    ...
    element.addEventListener("contextmenu", async (event) => {
      ...
      chrome.storage.local.get("flag_grid", (result) => {
        if (result.flag_grid.find((ele) => ele[0] === row && ele[1] === col)) {
          ...
          chrome.storage.local.set(
            ...
            () => {
              var curtain_cells = document.querySelectorAll(".curtain-cell");
          ...
          const new_flag_grid_array = [...result.flag_grid, [row, col]];
          chrome.storage.local.set(
            {
              flag_grid: new_flag_grid_array,
            },
  ...
  for (let i = 0; i < row; i++) {
    ...
    for (let j = 0; j < col; j++) {
      ...
      chrome.storage.local.get("mine_grid", function (result) {
        if (result.mine_grid) {
          if (result.mine_grid.find((ele) => ele[0] === i && ele[1] === j)) {
            element.classList.add("mine");
          } else {
            const mineCount = surroundMineCounter([i, j], result.mine_grid);
            if (mineCount > 0) {
...
function generateCurtain(table_class) {
  ...
  for (let i = 0; i < row; i++) {
    ...
    for (let j = 0; j < col; j++) {
      ...
      chrome.storage.local.get("clicked_grid", (result) => {
        if (result.clicked_grid.find((ele) => ele[0] === i && ele[1] === j)) {
          element.classList.add("transparent");
        }
      });
      chrome.storage.local.get("flag_grid", (result) => {
        if (result.flag_grid.find((ele) => ele[0] === i && ele[1] === j)) {
...
function mineLayer() {
  ...
  while (mine_count) {
    ...
    if (mine_grid.find((ele) => ele[0] === rand_row && ele[1] === rand_col)) {
      continue;
    } else {
